...
void game_lib_draw(void* data) {
    Camera2D camera = dte_game_camera();
    BeginMode2D(camera);
    dt_draw_handler_draw(handler);
    EndMode2D();
}

void game_lib_draw_destroy(void* data) { dt_draw_handler_destroy(handler); }
...
void load_draw_game_systems() {
    ...
    handler =
        dt_draw_handler_new(game_scene->manager, dt_vec_count(game_scene->draw_handler->systems));
    ...
    FOREACH(char*, name, DT_VEC_ITERATOR(game_scene->draw_handler->names), {
        ...
        for (int i = 0; i < draw->attribute_count; i++) {
            if (strcmp(DTE_EDIT_DRAW_NAME, draw->attributes[i].attribute_name) != 0)
                continue;

            dt_draw_handler_add(handler, ((DrawSystem * (*) ()) draw->attributes->data)(),
                                draw->name);
        }
    });
